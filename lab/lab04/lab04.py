    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            for k in range(1,amount//price+1):
                add(fruits[1:],amount-k*price, cart+"["+str(k)+" "+str(fruit)[:-1]+ "s]"if k>1 else cart+"["+str(k)+" "+str(fruit)[:-1]+ "]")
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    if (get_lat(city_a)-lat)**2+(get_lon(city_a)-lon)**2<(get_lat(city_b)-lat)**2+(get_lon(city_b)-lon)**2:
