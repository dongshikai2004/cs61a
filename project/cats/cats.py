...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def f(s):
        l=split(remove_punctuation(lower(s)))
        for i in l:
            if i in subject:
                return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for w in word_list:
        if w==typed_word:
            return w
        if diff_function(typed_word,w,limit)<l:
            pw=w
            l=diff_function(typed_word,w,limit)
    if l<=limit:
        return pw
    else:
        return typed_word
...
def feline_fixes(typed, source, limit):
    ...
    def f(t,s,cnt):
        if cnt>limit:
            return (999+limit)*1000
        if t=='' or s=='':
            return cnt
        if t[0]==s[0]:
            return f(t[1:],s[1:],cnt)
        return f(t[1:],s[1:],cnt+1)
    return f(typed,source,abs(len(typed)-len(source)))
    # END PROBLEM 6
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        ...
        for j in range(1,len(timestamps_per_player[0])):
            a.append(timestamps_per_player[i][j]-timestamps_per_player[i][j-1])
        times.append(a)
